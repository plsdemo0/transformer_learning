...
2. LEVEL 1 — Run automated scan:
   `python3 ~/.cursor/skills/aif-skill-generator/scripts/security-scan.py <skill-path>` when `PYTHON_CMD=(python3)`.
...
5. If BLOCKED at any level → run the cleanup helper with the same selected Python 3 command, for example `python3 ~/.cursor/skills/aif-skill-generator/scripts/cleanup-blocked-skill.py --skill <name> --installed-path <skill-path>` (reuse the same `<skill-path>` you passed to security-scan.py — upstream `skills` sanitizes the directory name on disk, so synthesizing `.cursor/skills/<name>` can miss the real folder; `--installed-path` lets the helper verify physical removal), report threats to user
```
...
3. **LEVEL 1** — Run automated scanner only when `PYTHON_CMD` is set:
   ...
   python3 ~/.cursor/skills/aif-skill-generator/scripts/security-scan.py <path>
   ```
...
```
If BLOCKED → run the selected concrete Python command with `~/.cursor/skills/aif-skill-generator/scripts/cleanup-blocked-skill.py --skill <name> --installed-path <installed-path>` (reuse the same `<installed-path>` you passed to security-scan.py — upstream `skills` sanitizes the directory name, so synthesizing `.cursor/skills/<name>` can miss the real folder; `--installed-path` lets the helper verify physical removal), warn. If WARNINGS → show to user.
...
```bash
# Example for PYTHON_CMD=(python3).
python3 ~/.cursor/skills/aif-skill-generator/scripts/security-scan.py ./skill-name/
```
