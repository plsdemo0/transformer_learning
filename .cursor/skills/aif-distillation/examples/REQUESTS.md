...
- `.cursor/skills/ddd-practices/SKILL.md`
- `.cursor/skills/ddd-practices/references/SOURCE-MAP.md`
...
- write each child directly under `.cursor/skills/<prefix>-<child-scope>/`
...
- no writes to the current agent `.cursor/skills`
...
- save to `.cursor/skills/platform-operator/`
...
- confirm the resolved target path would stay under the resolved output root (`.cursor/skills` by default, or `--path` when present)
